...
<template>
  <div class="flex items-center justify-center dark:text-white h-screen w-screen">
        ...
        <li v-for="product in products" :key="product.id"
          ...
          <span class="flex items-center justify-between w-full space-x-3">
            ...
            <span class="inline-flex items-center justify-between space-x-3">
              <button :disabled="product.quantity <= 0" class="bg-blue-400 hover:bg-blue-600 py-1 px-2 rounded-md"
                @click="decreaseQuantity(product)">
                -
              </button>
              <span class="w-12 text-center">{{ product.quantity }}</span>
              <button class="bg-blue-400 hover:bg-blue-600 py-1 px-2 rounded-md" @click="increaseQuantity(product)">
                +
              </button>
            </span>
